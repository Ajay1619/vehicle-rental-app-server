<?php
// vehicle_rental_app/modules/guest_registeration/create_guest_account.php
...
$username = isset($accountInfo['account_username']) ? sanitizeInput($accountInfo['account_username'], 'string') : null;
...
try {
    $check_username_existence_params_procedures = [
        ...
        ['name' => 'account_portal_type', 'type' => 'i', 'value' => 2]
    ];
    ...
    if ($check_username_existence_response) {
        if ($check_username_existence_response['particulars'][0]['status_code'] == 200) {
            $password = $accountInfo['account_password'] ?? null;
